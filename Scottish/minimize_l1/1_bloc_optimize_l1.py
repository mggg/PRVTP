import sys
from optimize_helper import generate_solid_profile
...
import pickle
from optimize_helper import l1_slate_profiles, l1_histograms
...
# diss_dict[f"{ward_label} real"] = diss_profile_to_solid(scottish_profile, cand_to_bloc, bloc_to_cand_num, bloc_order)
dist_dict[f"{ward_label} real"] = dist_profile_to_solid(scottish_profile, cand_to_bloc, bloc_order)
...
if model not in ["IC", "IAC"]:
    opt_stat = []
    pi_b_vector = []
    for pi_b in np.linspace(.01, .99, 99):
        ...
        if sum(bloc_to_cand_num.values()) >= 12 and model == "n-BT":
            ...
            profile = generate_solid_profile(pi_b, int(scottish_profile.total_ballot_wt), cand_to_bloc)
            
        else:
            profile  = bg.generate_profile(int(scottish_profile.total_ballot_wt))
           
    
        # l1 = l1_slate_profiles(profile, scottish_profile, cand_to_bloc)
        # diss_dict[f"pi_b {pi_b}"] = diss_profile_to_solid(profile, cand_to_bloc, bloc_to_cand_num, bloc_order)
        distances = dist_profile_to_solid(profile, cand_to_bloc, bloc_order)
        dist_dict[f"pi_b {pi_b}"] = distances
    ...
    # diss_dict[f"pi_b N/A"] = diss_profile_to_solid(profile, cand_to_bloc, bloc_to_cand_num, bloc_order)
    dist_dict[f"pi_b N/A"] = dist_profile_to_solid(profile, cand_to_bloc, bloc_order)
    min_pi_b = "N/A"
 


with open(f"data/1_bloc/{ward_label}_one_bloc_optimized_l1_{model}_bloc_order_{bloc_order}_b_bloc_parties_{b_bloc_parties}.pickle", "wb") as f:
    # min pi b, min l1, dist, diss
    # pickle.dump((min_pi_b, np.min(opt_stat), dist_dict[f"pi_b {min_pi_b}"],  diss_dict[f"pi_b {min_pi_b}"]),f)

    # min pi b, min l1, distances_model, distances_scottish
    pickle.dump((min_pi_b, np.min(opt_stat), dist_dict[f"pi_b {min_pi_b}"], dist_dict[f"{ward_label} real"]),f)
