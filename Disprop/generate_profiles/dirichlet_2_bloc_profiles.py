...
if "BT" in g:
    ...
    if b_coh>1 or b_coh<0 or a_coh>1 or a_coh<0:
        raise ValueError(f"cohesion must be in [0,1]: b={b_coh:.2f}, a={a_coh:.2f}")
...
path_name = f'saved_profiles/2_bloc/dirichlet/seats_{N_SEATS}/cperbloc_{N_CANDS_PER_BLOC}/trials_{N_TRIALS}/ballots_{N_BALLOTS}/fpv_b_{fpv_b}/fpv_a_{fpv_a}/b_prop_{b_prop}/{g}/pi_type_{pi_type}/dunif_{uniform_dirichlet}/dstrong_{strong_dirichlet}/'
path = Path(path_name)
...
for i in range(N_TRIALS):
    # fine that generate profile keeps the same PI the entire time because we only are counting slate candidates
    ...
    with open(f'{path_name}/seats_{N_SEATS}_cperbloc_{N_CANDS_PER_BLOC}_trials_{N_TRIALS}_ballots_{N_BALLOTS}_fpv_b_{fpv_b}_fpv_a_{fpv_a}_b_prop_{b_prop}_{g}_pi_type_{pi_type}_dunif_{uniform_dirichlet}_dstrong_{strong_dirichlet}_trial_{i}.pkl', 'wb') as file:
        pickle.dump(pp, file)
